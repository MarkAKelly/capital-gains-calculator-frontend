...
class CalculatorConnectorSpec extends UnitSpec with MockitoSugar {
  ...
  def mockResidentFetchAndGetFormData(): Unit = {
    when(mockSessionCache.fetchAndGetEntry[ReliefsModel](Matchers.eq(KeystoreKeys.ResidentKeys.reliefs))(Matchers.any(), Matchers.any()))
      .thenReturn(Future.successful(Some(mock[ReliefsModel])))

    when(mockSessionCache.fetchAndGetEntry[ReliefsValueModel](Matchers.eq(KeystoreKeys.ResidentKeys.reliefsValue))(Matchers.any(), Matchers.any()))
