...
class NoPrrReliefsFormSpec extends UnitSpec with WithFakeApplication {
  ...
  "Creating a form using an invalid map" when {
    ...
    "supplied with no data" should {
      ...
      s"return an error with message ${messages.errorSelectNoPrr(MoneyPounds(10000, 0).quantity)}" in {
        form.error("isClaiming").get.message shouldBe messages.errorSelectNoPrr(MoneyPounds(10000, 0).quantity)
      }
    }
