...
//                  resultsSet: Option[TotalGainAnswersModel]
...
//      .thenReturn(Future.successful(resultsSet.get))
//
...
//
//      val target = setupTarget(None, None, Some())
...
//    }
//
//    "supplied with a pre-existing model" in {
//      val target = setupTarget(Some(CalculationElectionModel("flat")), None)
//      lazy val result = target.calculationElection(fakeRequestWithSession)
//      lazy val document = Jsoup.parse(bodyOf(result))
//
//      "return a 200" in {
