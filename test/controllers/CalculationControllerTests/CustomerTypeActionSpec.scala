...
class CustomerTypeActionSpec extends UnitSpec with WithFakeApplication with MockitoSugar {
  ...
  "Have a dynamic back link generated by .customerType" when {
    ...
    "there is no acquisition date or rebased value" should {
      val target = setupTarget(None, None, Some(AcquisitionDateModel("No", None, None, None)), Some(RebasedValueModel("No", None)))
      lazy val result = target.customerType(fakeRequest)
      lazy val document = Jsoup.parse(bodyOf(result))
    ...
    "there is a acquisition date and/or rebased value" should {
      val target = setupTarget(None, None, Some(AcquisitionDateModel("Yes", None, None, None)), Some(RebasedValueModel("Yes", None)))
      lazy val result = target.customerType(fakeRequest)
      lazy val document = Jsoup.parse(bodyOf(result))
