...
class DisposalCostsActionSpec extends UnitSpec with WithFakeApplication with FakeRequestHelper with MockitoSugar {
  ...
  def setupTarget(getData: Option[DisposalCostsModel]): GainController = {
    ...
    when(mockCalcConnector.fetchAndGetFormData[DisposalCostsModel](Matchers.eq(KeystoreKeys.ResidentPropertyKeys.disposalCosts))(Matchers.any(), Matchers.any()))
  ...
  "calling .submitDisposalCosts from the GainCalculationController" when {
    ...
    "given a valid form should" should {
      ...
      s"redirect to '${controllers.resident.properties.routes.GainController.acquisitionValue().toString}'" in {
        redirectLocation(result).get shouldBe controllers.resident.properties.routes.GainController.acquisitionValue().toString
      }
    }
