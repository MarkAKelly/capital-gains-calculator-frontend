...
class PreviousTaxableGainsActionSpec extends UnitSpec with WithFakeApplication with FakeRequestHelper with MockitoSugar {
  ...
  "Calling .previousTaxableGains from the IncomeController" when {
    ...
    "other properties have been specified" should {
      "return a back link to the AEA page" in {
        ...
        link.attr("href") shouldBe controllers.resident.properties.routes.DeductionsController.annualExemptAmount().toString
      }
    }
    ...
    "other properties have been specified with a non-zero allowable loss" should {
      "return a back link to the losses brought forward page" in {
        ...
        val link = doc.select("#back-link")
        link.attr("href") shouldBe controllers.resident.properties.routes.DeductionsController.lossesBroughtForwardValue().toString
      }
    }
    ...
    "no other properties AND brought forward losses NOT specified" should {
      "return a back link to the brought forward choice page" in {
        ...
        val link = doc.select("#back-link")
        link.attr("href") shouldBe controllers.resident.properties.routes.DeductionsController.lossesBroughtForward().toString
      }
    }
  }
  ...
  "Calling .submitPreviousTaxableGains from the IncomeController" when {
    ...
    "a valid form is submitted" should {
      ...
      s"redirect to ${controllers.resident.properties.routes.IncomeController.currentIncome()}" in {
