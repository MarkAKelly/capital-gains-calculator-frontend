...
class OtherDisposalsViewSpec extends UnitSpec with WithFakeApplication with FakeRequestHelper {
  ...
  "Other Disposals view" when {
    ...
    "using a disposal date 0f 2015/16" should {
      ...
      lazy val view = views.otherDisposals(otherPropertiesForm, TaxYearModel("2015/16", true, "2015/16"), "home-link")(fakeRequest)
      ...
      "have a back button that" should {
        ...
        "have a link to acquisition costs" in {
          backLink.attr("href") shouldBe controllers.resident.shares.routes.GainController.acquisitionCosts().toString()
        }
      }
      ...
      "have a form" which {
        ...
        s"has the action '${controllers.resident.shares.routes.DeductionsController.submitOtherDisposals().toString}'" in {
          form.attr("action") shouldBe controllers.resident.shares.routes.DeductionsController.submitOtherDisposals().toString()
        }
    ...
    "Other Disposals view with pre-selected values" should {
      ...
      lazy val view = views.otherDisposals(form, TaxYearModel("2015/16", true, "2015/16"), "home-link")(fakeRequest)
      lazy val doc = Jsoup.parse(view.body)
