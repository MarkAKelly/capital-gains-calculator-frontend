...
class NoTaxToPayViewSpec extends UnitSpec with WithFakeApplication with FakeRequestHelper {
  ...
  "No Tax to Pay View when gifted to spouse" should {
    lazy val view = views.noTaxToPay(false, Some("back-link"), "home-link")(fakeRequest)
    ...
    "have a back link to back-link" in {
      doc.body().select("a#back-link").attr("href") shouldBe "back-link"
    }

    "have a home link to home-link" in {
      doc.body().select("a#homeNavHref").attr("href") shouldBe "home-link"
    }
  ...
  "No Tax to Pay View when gifted to charity" should {
    lazy val view = views.noTaxToPay(true, Some("back-link"), "home-link")(fakeRequest)
    lazy val doc = Jsoup.parse(view.body)
